...
__Separating simple linear regression models for assessing the association of total cholesterol level (cholesterol), total triglyceride level (triglyceride) and sinking pre-beta lipoprotein (spbl) with HDL (HDL) (mg/dL).__
...
The model explained only 18% of HDL and when compared with adjusted R-squared=11%, adding all three predictors did not efficiently improve the model’s predictability.


### Part 5) Variable added test

__Fitting a multivariable model and testing whether sinking pre-beta (spbl) is associated with HDL after the combined contribution of cholesterol and triglyceride levels is taken into account.__
...
__Evaluating if the association between cholesterol and HDL and the association between triglycerides and HDL depends on the absence/presence of SPBL.__
